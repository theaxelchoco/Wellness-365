        ...
        <ul className="mt-4">
            {Object.keys(groupedData).map((month) => (
                ...
                <ol className="mt-2 pl-2">
                    {groupedData[month]
                    .sort((a, b) => new Date(a.date).getUTCDate() - new Date(b.date).getUTCDate()) // Sort by day
                    .map((task, index) => (
                        <li key={index} className={`${inter.className} my-2`}>
                            <span className="text-sm text-black">{new Date(task.date).getUTCDate()}.</span> {task.task}
                        </li>
                    ))}
                </ol>
                </li>
